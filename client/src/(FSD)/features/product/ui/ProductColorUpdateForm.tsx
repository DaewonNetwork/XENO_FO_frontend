...
const ProductColorUpdateForm = () => {
    ...
    const { data } = useProductColorSizeStockRead(+productd)
    ...
    const onSubmit = (data: any) => {
        ...
        formData.append("productColorUpdateDTO", JSON.stringify({ productd: productd, size: sizeStocksToSend }));
    ...
    return (
        <>
            <form style={{ display: isOpen ? "none" : "block" }} className={styles.product_create_form} onSubmit={handleSubmit(onSubmit)}>
                ...
                <Button fullWidth size={"lg"} onClick={() => deleteProductColor(+productd)} >삭제하기</Button>
            </form>
